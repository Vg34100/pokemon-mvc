...
export class MoveModel {
  ...
  // New method: Fetch moves for a specific Pokémon
  async getMovesForPokemon(pokemonId: number): Promise<Move[]> {
    try {
      ...
      // Fetch detailed data for each move associated with the Pokémon
      const moves = await Promise.all(
      ...
      console.error(`Failed to fetch moves for Pokémon ID ${pokemonId}:`, error);
      return [];
    }
  }
}
